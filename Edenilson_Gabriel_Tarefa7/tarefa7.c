...
int visitado[50];
void verificaAntesDepois(int **p, int qtdVert, int vert_procurado);
...
void funcoes(){
	int i = 0, j = 0, opcao = 0, qtdVert = 0, count = 0, vertProcurado = 0;
    int **p = NULL;

    printf("Digite o numero de vertices:\n");
	scanf("%i", &qtdVert);
    printf("Digite o vertice que desejas buscar:\n");
	scanf("%i", &vertProcurado);
    ...
    verificaAntesDepois(p, qtdVert, vertProcurado-1);
...
void verificaAntesDepois(int **p, int qtdVert, int vertProcurado){
    int i, cont = 0, cont2 = 0, ligacoes[10], maior = 0;

    memset(ligacoes, 0, sizeof(ligacoes));

    for(i = 0; i < qtdVert; i++){
        if(p[vertProcurado][i] == 1){
            ligacoes[cont] = i+1;
            cont++;
        }
    }
